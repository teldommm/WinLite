...
size_t AifgPacer::MaxGenerations() const {
    if (config.multiplier < 2) return 0;
    if (config.target_rate != 0) return AIFG_MAX_MULTIPLIER - 1;
    return std::min<size_t>(config.multiplier, AIFG_MAX_MULTIPLIER) - 1;
}

void AifgPacer::TrackSourceRate(Clock::time_point now, uint64_t source_frames) {
...
void AifgPacer::TrackLoopRate(float interval_seconds) {
...
AifgPlan AifgPacer::Plan(size_t capacity, uint64_t source_frames) {
    ...
    limit = generations;
    return AifgPlan{generations, true};
}

AifgPacerStats AifgPacer::Stats() const {
    AifgPacerStats stats;
