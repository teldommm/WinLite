...
AifgGenerate::AifgGenerate(const Device& device, const AifgShaders& shaders,
                           AifgResources& resources, VkDescriptorPool descriptor_pool,
                           AifgImagePair& frames_, AifgImage& motion_, AifgImage& detail1_,
                           AifgImage& detail2_)
    : frames{&frames_}, motion{&motion_}, detail1{&detail1_}, detail2{&detail2_} {
    pass = AifgPass(device, shaders, AIFG_SHADER_GENERATE,
    ...
    const uint32_t total =
        static_cast<uint32_t>(AIFG_GENERATION_SLOTS * AIFG_MAX_TARGETS * 2);
    const std::vector<VkDescriptorSet> sets =
        AllocateAifgDescriptorSets(device, descriptor_pool, pass.SetLayout(), total);
    if (sets.size() != total) return;

    size_t next = 0;
    for (size_t slot = 0; slot < AIFG_GENERATION_SLOTS; ++slot) {
        Generation& target = generations[slot];
        target.buffer = resources.GetBuffer(AifgSlotTimestamp(slot));
...
void AifgGenerate::SetTarget(const Device& device, size_t slot, uint32_t target,
                             VkImageView view) {
    ...
    for (size_t i = 0; i < entry.descriptor_sets.size(); ++i) {
        AifgDescriptorWriter(entry.descriptor_sets[i])
            .AddUniformBuffer(generations[slot].buffer, AifgResources::BufferSize())
            .AddSampler(sampler)
...
void AifgGenerate::ForgetTargets() {
...
void AifgGenerate::Dispatch(VkCommandBuffer cmdbuf, uint64_t frame_count, size_t slot,
