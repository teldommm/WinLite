...
static_assert(sizeof(AifgConstants) == 48);

VkImageMemoryBarrier MakeBarrier(const AifgImage& image, VkAccessFlags src_access,
...
AifgImage::AifgImage(const Device& device_, VkExtent2D extent_, VkFormat format_)
...
AifgImage::AifgImage(AifgImage&& other) noexcept
...
AifgImage& AifgImage::operator=(AifgImage&& other) noexcept {
    if (this != &other) {
        Release();
        device = other.device;
        image = other.image;
...
VkDeviceSize AifgResources::BufferSize() {
    return sizeof(AifgConstants);
}

VkSampler AifgResources::GetSampler(VkSamplerAddressMode address_mode, VkCompareOp compare_op,
                                    bool white_border) {
    ...
    VkSampler sampler = CreateAifgSampler(*device, address_mode, compare_op, white_border);
    samplers.emplace(key, sampler);
    return sampler;
}

VkBuffer AifgResources::GetBuffer(float timestamp, bool first_iter, bool first_iter_s) {
    ...
    Buffer buffer{*device, sizeof(AifgConstants)};
    if (!buffer.Valid()) return VK_NULL_HANDLE;

    AifgConstants constants{};
...
AifgBarriers& AifgBarriers::Push(AifgImage& image, VkAccessFlags src_access,
                                 VkAccessFlags dst_access) {
...
AifgBarriers& AifgBarriers::WriteToRead(AifgImage& image) {
    return Push(image, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT);
}

AifgBarriers& AifgBarriers::ReadToWrite(AifgImage& image) {
...
AifgBarriers& AifgBarriers::DiscardToWrite(VkImage image) {
    VkImageMemoryBarrier barrier{};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    barrier.srcAccessMask = 0;
...
AifgDescriptorWriter& AifgDescriptorWriter::PushImage(VkDescriptorType type, VkSampler sampler,
                                                      VkImageView view) {
...
AifgDescriptorWriter& AifgDescriptorWriter::AddSampler(VkSampler sampler) {
    return PushImage(VK_DESCRIPTOR_TYPE_SAMPLER, sampler, VK_NULL_HANDLE);
}

AifgDescriptorWriter& AifgDescriptorWriter::AddSampledImage(const AifgImage& image) {
...
AifgDescriptorWriter& AifgDescriptorWriter::AddStorageImage(const AifgImage& image) {
    return PushImage(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_NULL_HANDLE, image.View());
}

AifgDescriptorWriter& AifgDescriptorWriter::AddStorageView(VkImageView view) {
    return PushImage(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_NULL_HANDLE, view);
}

AifgDescriptorWriter& AifgDescriptorWriter::AddUniformBuffer(VkBuffer buffer, VkDeviceSize size) {
...
void AifgDescriptorWriter::Build(const Device& device) {
...
AifgPass::AifgPass(const Device& device_, const AifgShaders& shaders, uint32_t shader_id,
...
AifgPass::AifgPass(AifgPass&& other) noexcept
...
AifgPass& AifgPass::operator=(AifgPass&& other) noexcept {
    if (this != &other) {
        Release();
        device = other.device;
...
void AifgPass::Bind(VkCommandBuffer cmdbuf, VkDescriptorSet set) const {
    BindPipeline(cmdbuf);
    BindSet(cmdbuf, set);
}

void AifgPass::BindPipeline(VkCommandBuffer cmdbuf) const {
    vkd.CmdBindPipeline(cmdbuf, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline);
}

void AifgPass::BindSet(VkCommandBuffer cmdbuf, VkDescriptorSet set) const {
    vkd.CmdBindDescriptorSets(cmdbuf, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout, 0, 1, &set, 0,
                            nullptr);
}

VkDescriptorPool CreateAifgDescriptorPool(const Device& device, uint32_t max_sets) {
...
std::vector<VkDescriptorSet> AllocateAifgDescriptorSets(const Device& device,
...
std::vector<VkDescriptorSet> AllocateAifgDescriptorSets(
...
VkSampler CreateAifgSampler(const Device& device, VkSamplerAddressMode address_mode,
