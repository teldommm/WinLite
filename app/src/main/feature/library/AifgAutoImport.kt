...
object AifgAutoImport {
    ...
    fun findDll(context: Context): File? {
        ...
        runCatching { AifgFrameGen.findInContainers(ContainerManager(context).containers) }
            .getOrDefault(emptyList())
            .forEach { candidates += it }

        return candidates.maxWithOrNull(
            compareBy<File> { AifgFrameGen.variantRank(AifgFrameGen.dllVariant(it)) }
                .thenBy { it.length() },
        )
    }
    ...
    fun sync(context: Context): Outcome {
        ...
        if (dll == null) {
            return Outcome(if (AifgFrameGen.isInstalled(context)) RESULT_READY else RESULT_NOT_FOUND, "")
        }

        val name = dll.parentFile?.name.orEmpty()
        val installed = AifgFrameGen.isInstalled(context)
        if (installed && !AifgFrameGen.isCacheStale(context, dll)) return Outcome(RESULT_READY, name)
